...
const buildTree = (fileData1, fileData2) => {
  ...
  const comparisonTree = keys.map((key) => {
    if (_.isPlainObject(fileData1[key]) && _.isPlainObject(fileData2[key])) {
      return { key, children: buildTree(fileData1[key], fileData2[key]), status: 'nested' };
    }
    if (!_.has(fileData1, key)) {
      return { key, value: fileData2[key], status: 'added' };
    } if (!_.has(fileData2, key)) {
      return { key, value: fileData1[key], status: 'removed' };
    ...
    return { key, value: fileData1[key], status: 'unchanged' };
  });
